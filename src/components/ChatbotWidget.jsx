...
// Separate Map Component to isolate Google Maps DOM from React
function PlacementMap({ address, dumpsterSize, onPositionChange }) {
  ...
  useEffect(() => {
    ...
    const initMap = async () => {
      ...
      if (!maps || !isActive || !containerRef.current) return
  ...
  }, [address, dumpsterSize, onPositionChange])

  // Empty div - React won't touch children added by Google Maps
  return <div ref={containerRef} className="h-72 md:h-96 w-full" />
}
...
export default function ChatbotWidget() {
  ...
  // Show map after a delay when entering MAP_PLACEMENT step
  useEffect(() => {
  ...
  const handlePlacementConfirm = async () => {
    ...
    setShowMap(false) // Hide map before moving to next step
  ...
  const handleGoBackToAddress = () => {
    setShowMap(false) // Hide map first
              ...
              {!isTyping && step === STEPS.MAP_PLACEMENT && (
                <div className="space-y-4 mt-4">
                  <div className="bg-dark-700 rounded-xl overflow-hidden">
                    {/* Map Container */}
                    <div className="relative bg-dark-600" style={{ minHeight: '300px' }}>
                      {showMap ? (
                        <PlacementMap
                          address={bookingData.address}
                          dumpsterSize={bookingData.size}
                  ...
                  <input
                    ...
                    placeholder="Add placement notes (optional)... e.g. 'Left side of driveway'"
