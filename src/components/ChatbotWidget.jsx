...
export default function ChatbotWidget() {
  ...
  // Initialize map when on MAP_PLACEMENT step
  useEffect(() => {
    if (step === STEPS.MAP_PLACEMENT && bookingData.address && mapRef.current) {
      initializeMap()
    }
    ...
    try {
      ...
      geocoder.geocode({ address: bookingData.address }, (results, status) => {
        if (status === 'OK' && results[0] && mapRef.current) {
          ...
          // Offset the rectangle from the marker
          ...
          setBookingData(prev => ({
            ...prev,
            placementLat: offsetLat,
            placementLng: offsetLng
          }))
  ...
  // Cleanup map when leaving placement step
  useEffect(() => {
  ...
  const handleGoBackToAddress = () => {
    // Remove the last two messages (user address + bot response)
        ...
        <div className="fixed inset-0 bg-dark-900 flex flex-col z-50">
          ...
          <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-4">
            <div className="max-w-2xl mx-auto">
              ...
              {/* Map Placement with Draggable Rectangle */}
              {!isTyping && step === STEPS.MAP_PLACEMENT && (
                <div className="space-y-4 mt-4">
                  <div className="bg-dark-700 rounded-xl overflow-hidden">
                    ...
                    >
                      {!mapReady && (
                        <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-4 z-10 bg-dark-600">
