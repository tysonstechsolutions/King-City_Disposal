...
export default function ChatbotWidget() {
  ...
  const getStaticMapUrl = () => {
    ...
    return `https://maps.googleapis.com/maps/api/staticmap?center=${address}&zoom=19&size=600x400&maptype=hybrid&markers=color:green%7C${address}&key=${GOOGLE_MAPS_KEY}`
  }
        ...
        <div className="fixed inset-0 bg-dark-900 flex flex-col z-50">
          ...
          <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-4">
            <div className="max-w-2xl mx-auto">
              ...
              {/* Map Placement - Using Static Image */}
                ...
                <div className="space-y-4 mt-4">
                  <div className="bg-dark-700 rounded-xl overflow-hidden">
                    ...
                    <div className="relative">
                      <img 
                        src={getStaticMapUrl()}
                        alt={`Satellite view of ${bookingData.address}`}
                      ...
                      />
                      <div className="hidden absolute inset-0 flex-col items-center justify-center text-center p-4 bg-dark-600">
                        <MapPin className="w-12 h-12 text-primary-400 mb-2" />
                        <p className="text-white font-medium">{bookingData.address}</p>
                      </div>
                      
                      {/* Dumpster size overlay */}
                      <div className="absolute bottom-3 left-3 bg-dark-900/90 backdrop-blur-sm rounded-lg px-3 py-2">
                        <div className="flex items-center gap-2">
                          <div className="w-5 h-3 bg-primary-500/60 border-2 border-primary-500 rounded-sm"></div>
                          <span className="text-white text-sm font-medium">
                            {selectedDumpster?.dimensions.display}
                          </span>
                        </div>
                      </div>
                    </div>
                    ...
                    {/* Address bar */}
                    <div className="p-3 bg-dark-800 border-t border-dark-600 flex items-center gap-2">
                      <MapPin className="w-4 h-4 text-primary-400 flex-shrink-0" />
                      <span className="text-sm text-dark-300 truncate">{bookingData.address}</span>
                    </div>
                  </div>
                  
                  <p className="text-dark-400 text-base">Where should we put the dumpster?</p>
                  ...
                  <input
                    type="text"
                    placeholder="Or type specific instructions..."
                    className="input-field w-full"
                    onKeyDown={(e) => {
                      if (e.key === 'Enter' && e.target.value.trim()) {
                        handlePlacementConfirm(e.target.value.trim())
                      }
                    }}
                  />
                </div>
